...
const Board = () => {
  ...
  const rejectRequest = async (tripId) => {
    try {
      const response = await axios.post(`http://localhost:8080/trip/rejectRequest`);
      const trips = response.data
      console.log(trips);
                    ...
                    {tripRequests.map((request) => (
                        <div key={request.id} className="card" >
                          ...
                          <div className="checks">
                            <ImCheckboxChecked size={23} color="green" onClick={() => acceptRequest(request.senderId)}/>
                            <GiCancel size={23} color="red"  onClick={() => rejectRequest(request.id)}/>
                          </div>
                        </div>
                    ))}
                  </ul></div>
